...
export async function generateMetadata(
  ...
  try {
    ...
    return {
      title: `${businessName || name} | Beauty Professional | Glamyad`,
      description:
      ...
      openGraph: {
        ...
        url: `https://glamyad.com/pro/${slug}`,
...
export default async function PublicProviderProfilePage({
  ...
  try {
    // We use the same service but we should ensure it handles slugs (which it does based on backend investigation)
    const provider = await providerService.getProviderById(slug);
    ...
    const startingPrice =
      services?.length > 0 ? Math.min(...services.map((s) => s.price)) : 500000;
        ...
        <div className="bg-white pb-24">
          ...
          <div className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
            <div className="flex flex-col gap-12 lg:flex-row lg:items-start">
              ...
              <aside className="w-full space-y-8 lg:w-96 lg:shrink-0 lg:sticky lg:top-24">
                <div className="rounded-2xl border border-slate-200 bg-white p-8 border-t-4 border-t-slate-900 shadow-sm">
                  ...
                  <Link
                    href={`/book/${slug}`}
                    className="mt-8 flex justify-center w-full rounded-full bg-slate-900 py-4 text-lg font-black text-white transition-all hover:bg-slate-800 active:scale-95 shadow-lg shadow-slate-200"
                  >
                    Book now
                  </Link>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200 bg-white/80 p-4 backdrop-blur-lg md:hidden">
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
              ...
              <Link
                href={`/book/${slug}`}
                className="flex-1 rounded-full bg-slate-900 py-3 font-bold text-white transition-all active:scale-95 text-center px-4"
              >
                Book Appointment
              </Link>
            </div>
