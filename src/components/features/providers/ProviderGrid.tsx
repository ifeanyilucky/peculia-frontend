...
export default function ProviderGrid({
    ...
    return (
      <div className="grid gap-6 sm:grid-cols-2">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="h-[400px] animate-pulse rounded-2xl bg-glam-blush/20"
          />
        ))}
      </div>
    );
  ...
  if (isError) {
    return (
      <div className="flex h-[400px] flex-col items-center justify-center text-center">
        <p className="text-glam-plum font-bold">
  ...
  if (allProviders.length === 0) {
    return (
      <div className="flex h-[400px] flex-col items-center justify-center text-center p-12 bg-glam-blush/20 rounded-3xl border border-dashed border-glam-blush">
        <SearchX size={48} className="text-glam-plum/40" />
        <h3 className="mt-4 font-peculiar text-xl font-bold text-glam-plum">
          No professionals found
        </h3>
        <p className="mt-2 text-glam-charcoal/60 max-w-xs">
